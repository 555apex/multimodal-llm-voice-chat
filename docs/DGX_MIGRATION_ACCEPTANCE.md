...
## 10. 数字人嘴型与姿态连续化（2026-09-04）

| 检查 | 结果 | 证据摘要 |
|---|---|---|
| 三档嘴型资产 | 通过 | 闭嘴、半开、张开均为 1024×1536 RGBA PNG/无损 WebP；只在固定嘴部 ROI 内变化，区域外像素和母版透明通道完全一致 |
| 音频振幅驱动 | 通过 | 实际 MP3 播放使用 `AudioContext + AnalyserNode` 获取 0..1 音量包络；噪声门限、归一化、上升/回落平滑及 30 fps 上限均已接入 |
| 嘴型连续混合 | 通过 | 0–0.45 在闭嘴/半开间插值，0.45–1 在半开/张开间插值；70 ms 视觉缓动；暂停、结束、失败和资源释放时归零 |
| 安全降级 | 通过 | Web Audio 不可用时仅在真实播放期间启用低幅节奏；嘴型资产失败时回退原讲解姿态，不影响音频播放和文字业务 |
| 姿态连续化 | 通过 | 三姿态始终挂载，600 ms `cubic-bezier(.22, 1, .36, 1)` 透明度/位移/缩放/模糊过渡；同姿态业务状态不更换图片 |
| 快速切换 | 通过 | 自动完成 20 轮、120 次状态切换；三姿态 DOM 层始终存在，无加载占位或素材回退，最终姿态正确收敛 |
| 减少动态偏好 | 通过 | 关闭身体循环位移和模糊移动，姿态仅 120 ms 淡入；语音状态使用静态半开嘴提示 |
