    ...
    def handle_message(data):
        ...
        try:
            if msg_type == 'text':
                ...
                process_text_message(client_id, user_text, auto_read)

            elif msg_type == 'audio':
                # 语音消息
                audio_data = data.get('content', '')
                audio_format = data.get('format', 'wav')
                process_audio_message(client_id, audio_data, audio_format, auto_read)
...
def process_audio_message(client_id, audio_data, audio_format, auto_read):
    """处理语音消息"""
    # ASR 识别
