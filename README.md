...
> **roadagent-v2 · DGX 运行源码发布版（2026-09-09）**：本分支取自 DGX 当前部署目录，业务基准为 `61e64d4`，叠加数字人和本地 Qwen3.8 部署。先阅读 [v2 更新与使用说明](README_ROADAGENT_V2.md) 和 [发布日志](CHANGELOG.md)。
> 当前 LLM 为 **Qwen3.8-27B-FP8**，ASR 为 faster-whisper small，TTS 为 Qwen3-TTS 0.6B Serena；保留数字人动作与音频驱动口型。模型只在服务端切换，不增加前端模型选择器。
> 当前模型配置和回滚见 [Qwen3.8 指南](Qwen3.8_DGX部署与模型切换指南.md)；模型注册表为 [model-serving/configs/models.yaml](model-serving/configs/models.yaml)。[原合并记录](docs/DGX_MERGE_20260908.md) 和根目录 `configs/models.yaml` 保留历史合并时的 Qwen3.6 信息，不代表当前运行模型。
