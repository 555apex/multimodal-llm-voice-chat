...
> **DGX 合并部署版本（业务基准 `4dab572`）**：DGX 继续使用本地 Qwen3.6、Qwen3-TTS 和 faster-whisper small，并保留数字人动作、口型、公网入口和登录方式。数据库切换只通过受保护的运行环境文件完成，禁止执行演示数据重置脚本。

DGX 当前 `roadagent-v2` 源码、运行镜像对应关系及脱敏边界见 [2026-09-14 部署源码基线](docs/DGX_BASELINE_20260914.md)。

## 2026-09-12 预案资源解耦与生成恢复加固

- 相较上一版提交 `53498de`，本版的主要变化是解除应急预案对资源类型和数量的静态限制、补强模型输出纠错与失败恢复，并将道路通行能力的“瓶颈/严重瓶颈”阈值由20%/30%修正为60%/80%。本版没有新增数据库表。
- 16类预案只约束事件类型、待核实事实和处置正文，不再设定资源类型或数量基线。模型可根据事件与人工返工意见，从当前启用的数据库资源目录中提出需求；Java仍负责库存、距离、保有量和缺口计算。
- 模型首次返回目录外编码、重复类型、非法数量或不合格正文时，核心服务会带具体原因自动修正一次；仍失败则落为可恢复的 `GENERATION_FAILED`，不进入库存事务。
- 人工审阅Word已改为“需核实信息 + 简明处置预案”，删除资源基线表格。共享库已由负责人执行 [无资源基线预案发布脚本](docs/sql/20260912_publish_resource_unbound_response_plans.sql)，16类活动预案均为v3且 `resource_baseline=[]`；旧版、工单和通告快照保留。普通协作者不要重复发布。
...
### 4.2 2026-09-12“预案资源解耦”发布

`docs/sql/20260912_publish_resource_unbound_response_plans.sql` 在简明v2基础上为16类事件各发布一个新版本，继承 `required_facts` 和 `rescue_plan_template`，将 `resource_baseline` 设为空JSON数组，并写入 `source_document='resource-unbound-20260912'`。脚本保留旧版及所有工单快照，不改动事件、通告、库存或占用记录；它会拒绝在生成或返工中切换预案，并对16类事件、部分发布和活动版基线进行事务校验。

`docs/福建公路应急事件分类与处置预案草案.docx` 是供业务人员审阅的16类预案源稿，应用运行时不会读取该Word。`docs/tools/generate_response_plan_review.py` 是维护审阅稿的辅助工具，它从现有Word保留各类“需核实的现场信息”，再读取测试资源中的简明预案文本生成新版文档；普通协作者启动项目时不需要运行它，也不需要安装 Python 文档依赖。

项目共享 `road_agent` schema 已于2026-09-12完成该发布，当前16个活动预案均为v3。协作者拉取代码后不需要执行脚本，只需确认共享库结果：

```sql
SELECT COUNT(*) AS active_count,
       COUNT(DISTINCT event_type) AS active_type_count,
       SUM(source_document = 'resource-unbound-20260912') AS unbound_active_count,
       SUM(JSON_LENGTH(resource_baseline) = 0) AS empty_baseline_count
FROM w_emergency_response_plan
WHERE plan_status = 1;
```

四个计数都应为16。日后需在新数据库独立发布时，必须先完成2026-09-04建表与v1发布、2026-09-10简明v2发布，再由数据库负责人统一执行本脚本。
...
- `w_emergency_response_plan`：按事件类型保存已发布预案版本和固定模板；旧版 `resource_baseline` 仅为历史快照，不再限制运行时资源；
...
16类事件的人工审阅源稿位于 `docs/福建公路应急事件分类与处置预案草案.docx`。运行时不读取Word，只读取 `w_emergency_response_plan` 中与事件类型对应的唯一已发布版本。模型根据预案主体、事件事实和人工意见生成处置方案，并可从全部启用的数据库资源目录中提出实际需求；Java验证目录编码，按库存和距离匹配，并将预案与资源结果快照冻结到工单和最终通告。
...
- 模型根据事件事实和人工返工意见填充对应类型的已发布预案，并从全部启用的数据库资源目录中提出需求；预案中的旧资源基线和资源的历史适用类型标签不再构成运行时限制；
- 模型不能编造资源 ID、来源城市、距离或库存；首次返回数据库目录外编码、重复编码、非法数量或不合格预案文本时，Core会携带具体校验原因自动修正一次，仍不合格才进入可重试的 `GENERATION_FAILED`；
